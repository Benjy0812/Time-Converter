...
def main():  # main function
    ...
    while True:  # run the program until the user exits
        ...
        if (
            time_value is None
        ):  # If the input is invalid, go back to the start of the loop
            continue  # go back to the start of the loop

        if (
            conversion_choice == 8
        ):  # If it's normal hours and minutes to fractional, get minutes
            try:  # get the number of minutes
                ...
                continue  # go back to the start of the loop
            result, unit = time_convert(
                conversion_choice, time_value, minutes
            )  # Call the function with minutes
        else:  # Otherwise
            result, unit = time_convert(
                conversion_choice, time_value
            )  # Call the function without minutes

        if result is not None:  # valid option
            if (
                conversion_choice == 7
            ):  # If it's fractional hours to normal hours and minutes
